...
public class MainActivity extends AppCompatActivity {
	...
	private TextView textViewSystemDateTile, textViewBottomMessage, textViewListViewMessage, textViewInspection,
			textViewRecharge, textViewDeviceID, textViewAppOrgan, textViewSwitch;
	...
	BaseApplication.SerialportCom0 serialportCom0 = new BaseApplication.SerialportCom0() {
		...
		public void SerialportListener(String comStr) {
			try {
				if (comStr.indexOf("startcheck") != -1) { //开始检测
					if (baseApp.getRechargeNum() >= 1) { //剩余检测次数大于等于1时可以开始检测
						com0.sendTxt("alreadycheck");
						noprint=1;
						if (isPcQuest == 1) {
							isPcQuest = 2;
						...
						com0.sendTxt("sample-s_2,2_sample-e"); //剩余检测次数不足

					}

				}else if (comStr.indexOf("port-s_00_port-e") != -1){
					//联机开关打开
					baseApp.can_link = "1";
					textViewSwitch =  (TextView) findViewById(R.id.TextView01);
					textViewSwitch.setBackground(getResources().getDrawable(R.drawable.butn_open));

				}else if (comStr.indexOf("check-s_startselfcheck_check-e") != -1){ //开始环境自适应
				...
				} else if (comStr.indexOf("sample-s_getstate_sample-e") != -1) { //适用FR9202型号
	...
	BaseApplication.SerialportCom serialportCom = new BaseApplication.SerialportCom() {
		...
		public void SerialportListener(String comStr) {
			...
			try {
				String[] datas = comStr.split(" "); //接收的字符串转换为数组
				String cmd = datas[2];
				comstring = comStr;
				int dataLength = Integer.parseInt(datas[1]);
				Message msg = serialPortHandler.obtainMessage();

				if (cmd.equals("1A")) { //单片机上传呼吸卡状态
					msg.arg1 = 1;
				} else if (cmd.equals("18")) { //充值使用次数
					// DD 02 18 00 0A 10
					String rechargeNum = "";
					...
					com0.sendTxt("check-s_out_check-e"); //通知上位机呼吸卡被正常取出
					// DD 01 1C 00 1D ED 呼吸卡被拔出
					//isPcQuest = 0;
					msg.arg1 = 3;
				} else if (cmd.equals("33")) {  //呼吸卡取出异常
					msg.arg1 = 5;
					...
					if (!allow1.equals("FFFF")) {
						com1.sendHex("AA01920093ED"); //每次检测结束后校正时间//
					}
					msg.arg1 = 4;
					//textViewBottomMessage.setText(comStr);
					...
					if (passageway.equals("00")) {
						...
						td = false;
						//com1.sendHex("AA01920093ED"); //校正时间
...
//					lock = false;
					...
					textViewBottomMessage.setText("呼吸卡取出异常");
					com0.sendTxt("check-s_outputabnormal_check-e");
					com1.sendHex("AA01290028ED"); //结束之前进程·终止当前检测·UI和其他状态指示   告知上位机
					textViewInspection.setText("开始检测");
					chronometer.setVisibility(View.GONE);
					inspectionImg.setImageResource(R.drawable.inspection_end);
					inspectionImg.clearAnimation();
					...
					// 通知上位机呼吸卡取出异常
					// com0.sendTxt("sample-s_isout_sample-e");
	...
	private void init() {
		...
		textViewRecharge.setText("剩余：" + baseApp.getRechargeNum() + "次");
		textViewDeviceID.setText("设备ID：" + baseApp.getDeviceId()); //增加4个空格


		textViewSwitch =  (TextView) findViewById(R.id.TextView01); //联机开关按钮

		if(baseApp.can_link == "1"){
			textViewSwitch.setBackground(this.getResources().getDrawable(R.drawable.butn_open)); //开启状态

		}else{
			textViewSwitch.setBackground(this.getResources().getDrawable(R.drawable.butn_close)); //关闭状态

		}
		textViewSwitch.setOnClickListener(new View.OnClickListener() { //关闭按钮监听
			...
			public void onClick(View v) {
				...
				if(baseApp.can_link == "1"){
					//isPcQuest = 0;
					textViewSwitch =  (TextView) findViewById(R.id.TextView01);
					textViewSwitch.setBackground(getResources().getDrawable(R.drawable.butn_close));
					baseApp.can_link = "0";
				}
