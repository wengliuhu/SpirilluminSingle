...
//			resetime();
	...
	public void Print(HashMap<String, Object> hashMap) {
		...
		if (com2.isOpen()) {
			...
			com2.sendHex("0A");
			com2.sendHex("0A");
			com2.sendTxt("检验人 : " + hashMap.get("OPERATOR"));
...
//			rechargeNum = sharedPreferences.getInt("rechargeNum", 80);
			rechargeNum = sharedPreferences.getInt("rechargeNum", 40);
...
//				}
	...
	public void SendParmasToPc() {
		try {
			...
			// TODO: 2020/5/16 0016 ，两个版本，一个是deviceid,另一个是FR9102
			// FR9102
			com0.sendTxt("param-s_" + ((inspectionDate.getHours() * 60) + inspectionDate.getMinutes()) + "_"
					...
					+ getYangxing3() + "_" + new_bdlx + "_" + "FR9102" + "_param-e"); //增加双通道机器型号FR9202
